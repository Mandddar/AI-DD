    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {stats.map(({ label, value, icon: Icon, color, bg, ring, trend }) => (
          <div key={label} className="card p-6 group hover:border-canvas-border/80 transition-all">
            <div className="flex items-start justify-between mb-4">
              <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${bg} ring-1 ${ring}`}>
            ...
            <p className="mt-1 text-sm text-text-muted uppercase tracking-wider">{label}</p>
            <p className="mt-2 text-sm text-text-secondary border-t border-canvas-border pt-2">{trend}</p>
          </div>
        ))}
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between border-b border-canvas-border px-6 py-5">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">Your latest active mandates</p>
          </div>
          <Link
            to="/projects"
          ...
          <div className="divide-y divide-canvas-border">
            {projects.slice(0, 6).map((project) => (
              <Link
                ...
                className="group flex items-center justify-between px-6 py-5 hover:bg-surface/40 transition-colors"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-medium text-text-primary group-hover:text-gold transition-colors">
                      {project.name}
                    </p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <p className="text-sm text-text-muted">{project.company_name}</p>
                      <span className="text-text-muted/40">·</span>
                      <p className="text-sm text-text-muted">{project.legal_form}</p>
                      <span className="text-text-muted/40">·</span>
          ...
          <div className={`grid gap-4 ${quickActions.length === 3 ? 'grid-cols-3' : quickActions.length === 2 ? 'grid-cols-2' : 'grid-cols-1 max-w-sm'}`}>
            {quickActions.map(({ icon: Icon, label, sub, to, color, bg }) => (
              ...
              >
                ...
                <div>
                  <p className="text-base font-medium text-text-primary group-hover:text-gold transition-colors">{label}</p>
