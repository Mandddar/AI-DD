    ...
    <div className="p-6 space-y-6 animate-fade-in max-w-3xl">
      ...
      <div className="card p-6 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2">
          {profileMsg && <p className={`text-xs ${profileMsg.includes('success') ? 'text-risk-low' : 'text-risk-high'}`}>{profileMsg}</p>}
          <button
      ...
      <div className="card p-6 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2">
          {pwdMsg && <p className={`text-xs ${pwdMsg.includes('success') ? 'text-risk-low' : 'text-risk-high'}`}>{pwdMsg}</p>}
          <button
            className="btn-primary px-4 py-2 text-sm flex items-center gap-2 ml-auto"
      ...
      <div className="card p-6 space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="text-lg font-display font-semibold text-text-primary">Two-Factor Authentication</h2>
          {user?.totp_enabled && (
            <span className="ml-2 inline-flex items-center rounded-full bg-risk-low/10 px-2 py-0.5 text-xs font-medium text-risk-low">
              Enabled
            </span>
          )}
        </div>
          ...
          <>
            {totpSecret ? (
              <div className="space-y-3">
                <div className="bg-surface rounded-lg border border-canvas-border p-4">
                  <p className="text-xs text-text-muted mb-2">Manual entry key:</p>
                  <code className="text-sm text-gold font-mono break-all">{totpSecret}</code>
                </div>
                <div className="bg-surface rounded-lg border border-canvas-border p-4">
                  <p className="text-xs text-text-muted mb-2">Provisioning URI (paste in authenticator):</p>
                  <code className="text-xs text-text-secondary font-mono break-all">{totpUri}</code>
                </div>
                <div className="flex items-center gap-3">
                  <input
        ...
        {totpMsg && <p className={`text-xs ${totpMsg.includes('success') || totpMsg.includes('enabled') ? 'text-risk-low' : 'text-text-secondary'}`}>{totpMsg}</p>}
      </div>
      ...
      <div className="card p-6 space-y-4 border-risk-high/20">
        <div className="flex items-center gap-2 mb-2">
          <AlertTriangle className="w-5 h-5 text-risk-high" />
          <h2 className="text-lg font-display font-semibold text-risk-high">Delete Account</h2>
        </div>
        ...
        <div className="space-y-3">
          ...
          {deleteMsg && <p className="text-xs text-risk-high">{deleteMsg}</p>}
        </div>
      </div>
    </div>
  );
